...
class TelegramAdapter(BasePlatformAdapter):
    ...
    async def _cache_inbound_av(self, msg, event: MessageEvent, source: Any, label: str, kind: str, ext: str, mime: str) -> bool:
        ...
        try:
            ...
            if not allowed:
                ...
                await self.handle_message(event)
                return True
            file_obj, data = await self._download_telegram_media(source)
            if kind == "video":
    ...
    async def _cache_inbound_document(self, msg, event: MessageEvent) -> bool:
        ...
        try:
            ...
            file_obj, downloaded = await self._download_telegram_media(doc)
    ...
    async def _handle_media_message(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        ...
        if msg.photo:
            try:
                file_obj, image_bytes = await self._download_telegram_media(msg.photo[-1])
